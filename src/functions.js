...
async function getAllVideos(playlist) {
  ...
  let { data: challenges } = await axios.get(url, { auth: { username: "dipamsen" } })
...
async function getVideo(playlist, index) {
  ...
  try {
    ...
    let { data: allVideoData } = await axios.get(url, { auth: { username: "dipamsen" } })
    ...
    const {
      ...
      videos,
      web_editor,
      ...restVideoProperties  // remove unnecessary properties from the object
    } = video
    let contributions;
    if (restVideoProperties.contributions) contributions = restVideoProperties.contributions
    else if (Math.floor(index) == index) contributions = []
    else contributions = (await getVideo(playlist, Math.floor(index) + 0.1)).contributions
    return {
      ...restVideoProperties,
...
const randomContribution = async (playlist, baseURL) => {
  ...
  if (typeof video !== "object") return "error"
    ...
    if (video.contributions && typeof video.contributions == 'object') return {
      ...randomArr(video.contributions),
      originalVideo: ogVideoData
    }
    else if (Math.floor(videoIndex) == videoIndex) return await randomContribution(playlist, baseURL)
    else {
...
module.exports = { getAllVideos, getVideo, randomContribution, allPlaylists, randomArr }